import cv2
import logging
from droneblocksutils.aruco_utils import detect_markers_in_image, detect_distance_from_image_center, detect_faces_in_image
...
def handler(tello, frame, fly_flag=False):
    ...
    # If you get here, we should try to detect faces in the video frame.
    # image, marker_details = detect_markers_in_image(frame, draw_center=True, draw_reference_corner=True,
    #                                                 target_id=ARUCO_TARGET_ID)
    image, marker_details = detect_faces_in_image(frame, draw_center=True, draw_reference_corner=True)
